...
export default function InformationModal({ informationModalOpen, setInformationModalOpen, informationModalType, informationModalMessage }) {
    ...
    if(informationModalType == "wrong_position") {
        modalTitle = translate("informationModal.wrongPosition")
        playerName = modalMessage[0]
        originalPosition = translate(`position.${modalMessage[1]}`)
        positionToPut = translate(`position.${modalMessage[2]}`) 
    }
                ...
                <Modal.Body>
                    <div className="text-center">
                        ...
                        <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400 font-sans">
                            {`${playerName} ${translate("informationModal.playersPosition")} ${originalPosition}${
                                translate("informationModal.positionError").replace("{}",positionToPut)}`}
                        </h3>
                    </div>
                </Modal.Body>
                <Modal.Footer>
                    ...
                    text-center me-2 mb-2" onClick={() => setInformationModalOpen(false)}>{translate("playerSelectModal.closeModal")}</Button>
                </Modal.Footer>
            </Modal>
    );
}