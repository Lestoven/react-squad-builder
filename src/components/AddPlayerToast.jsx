...
function AddPlayerToast({ isOpen, setIsOpen, player, setSelectedPlayerFromBench }) {
    const toggleToast = () => {
        setIsOpen(!isOpen)
    };
        ...
        <div className="addPlayer fixed bottom-4 right-4 z-50">
            {isOpen && (
                <div className="bg-white min-w-500px border-white p-5 rounded-md shadow-md flex flex-col items-start items-center justify-between max-w-72">
                        <h4 className="mb-4 text-md font-bold dark:text-white break-words">{translate("addPlayerToast.addingToSquad")} <br />
                        ...
                        dark:border-gray-700 flex flex-col" style={{ background: 'linear-gradient(rgb(0,200, 228), rgb(30, 40, 200))' }}>
                            ...
                            <div className="p-2 flex flex-col items-center flex-grow">
                                <a className="text-center">
                                    ...
                                    <h6 className="text-white">({translate(`position.${player["positionType"]}`)})</h6>
                                </a>
                            </div>
                        ...
                        <p>{translate("addPlayerToast.cancelAddingPlayer")}</p>
                    </button>
                </div>
            ...
            {player && 
                ...
                className="mt-1 bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-full shadow-md focus:outline-none">
                    {!isOpen ? 
                        translate("addPlayerToast.showSelectedPlayer")
                    :
                        translate("addPlayerToast.hideSelectedPlayer")
                    }
                </button>
            }
